...
const filePath = path.join(__dirname, '..', 'components', 'StudentHome.js');
let content = fs.readFileSync(filePath, 'utf8');

const startMarker = "    // Classroom simulator logic (popups/simulations)\r\n    launchClassroomSimulator(activeClass) {";
const startMarkerUnix = "    // Classroom simulator logic (popups/simulations)\n    launchClassroomSimulator(activeClass) {";
const endMarker = "    // View submitted student homework PDF\r\n    launchHomeworkViewer(hw) {";
const endMarkerUnix = "    // View submitted student homework PDF\n    launchHomeworkViewer(hw) {";
...
if (startIdx === -1) {
    console.error("Error: start_marker not found");
    process.exit(1);
}
if (endIdx === -1) {
    console.error("Error: end_marker not found");
    process.exit(1);
}

const newImpl = `    // Classroom simulator logic (popups/simulations)
    launchClassroomSimulator(activeClass) {
...
const updatedContent = content.substring(0, startIdx) + newImpl + content.substring(endIdx);
fs.writeFileSync(filePath, updatedContent, 'utf8');
console.log("Replacement successful via JS script!");
