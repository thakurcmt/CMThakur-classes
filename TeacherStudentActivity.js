...
EduApp.TeacherStudentActivity = {
    ...
    renderHomeworkTab(students) {
        ...
        if (createBtn) {
            createBtn.addEventListener('click', async () => {
                ...
                await EduApp.db.createHomeworkAssignment(title.trim(), subject.trim());
    ...
    renderAttendanceTab(students) {
        ...
        if (saveAttendBtn && students.length > 0) {
            saveAttendBtn.addEventListener('click', async () => {
                const todaySheetObj = {};
                const todaySheetArr = [];
                let presentCount = 0;
                ...
                if (typeof EduApp.db.saveAttendance === 'function') {
                    await EduApp.db.saveAttendance(todaySheetArr);
                }
